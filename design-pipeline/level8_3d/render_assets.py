"""Render the new Level 8's game assets with Blender (run with Blender's Python, see README).
...
Same scene, camera and lights as the approved mockup (scene.py).
"""
...
for k in roots:
    ...
    bpy.ops.render.render(write_still=True)
    json.dump({'hole': k, 'variant': CAST[k], 'x0': 2 * x0, 'y0': 2 * y0, 'x1': 2 * x1, 'y1': 2 * y1},
              open(os.path.join(OUT, 'char_%s.json' % k), 'w'))
print('assets done')
