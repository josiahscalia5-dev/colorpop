...
opening = json.load(open(work('lvl_front_edges.json')))['opening']
chars = json.load(open(OUT + '/characters/_meta.json'))['chars']
COLOR = {'g': 'green', 'r': 'red', 'y': 'yellow'}
p = hud['pause']
level = {
    'art': {'w': ground['art_w'], 'h': ground['art_h']},
    'bg': {'file': 'level/bg.png', 'pad_side': ground['pad_side'], 'pad_bottom': ground['pad_bottom']},
    ...
    # opening ellipse [cx, cy, a, b]: its lower half is the front edge a character disappears behind
    'holes': {h: {'opening': opening[h]} for h in sorted(opening)},
